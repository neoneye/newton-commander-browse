...
typedef void (^IPCManagerDidReceiveDataBlock)(NSData* data);

@interface IPCManager : NSThread

-(id)initWithDidReceiveDataBlock:(IPCManagerDidReceiveDataBlock)didReceiveDataBlock;
...
@property (nonatomic, copy) IPCManagerDidReceiveDataBlock didReceiveDataBlock;
@property (nonatomic, strong) ZMQContext *context;
@property (nonatomic, strong) ZMQSocket *socket;

@end

@implementation IPCManager
...
@property(nonatomic, strong) IPCManager *ipcManager;
...
-(void)createConnectionWithZeroMQ {
	NSAssert(_ipcManager == nil, @"_zmqContext must not already be initialized");
	
	IPCManagerDidReceiveDataBlock didReceiveDataBlock = ^(NSData* data) {
		NSLog(@"yay");
		sleep(1);
	};

	
	IPCManager *manager = [[IPCManager alloc] initWithDidReceiveDataBlock:didReceiveDataBlock];
